...
**Entry point:** `AGENTS.md` (Vietnamese, UTF-8). **Real app paths** � dev: `apps/main/*`; deploy lines under `apps/hub-parent/*`, `apps/hub-checkin/*`, `apps/store-sync/*`. Legacy shorthand `apps/api` / `apps/backend` / `apps/frontend` must not be used for new work.
