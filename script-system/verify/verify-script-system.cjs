...
if (fs.existsSync(path.join(SCRIPT_SYSTEM, "deploy"))) {
  fail("script-system/deploy/ đã bỏ — deploy script thuộc downstream product")
}
...
if (fs.existsSync(path.join(SCRIPT_SYSTEM, "hub-parent"))) {
  fail("script-system/hub-parent/ — dùng apps/<line>/api/scripts/apply-product-overrides.cjs")
}
...
if (fs.existsSync(syncRoot)) {
  for (const entry of fs.readdirSync(syncRoot, { withFileTypes: true })) {
    ...
    if (!SYNC_ROOT_SCRIPTS.has(entry.name)) {
      fail(`script-system/sync/${entry.name} — xóa file legacy hoặc cập nhật allowlist`)
    }
  }
}
...
const REMOVED_VERIFY = new Set([
  "verify-api-profile.cjs",
  "verify-service-boundaries.cjs",
])

for (const entry of fs.readdirSync(VERIFY_DIR)) {
  if (entry.endsWith(".mjs")) {
    fail(`verify/ còn file .mjs (chuyển sang .cjs): script-system/verify/${entry}`)
  }
  if (REMOVED_VERIFY.has(entry)) {
    fail(`script-system/verify/${entry} — đã chuyển sang packages/*`)
  }
}
...
console.log(
  `verify:scripts OK — ${ALLOWED_TOP_DIRS.size} nhóm thư mục; package scripts nằm trong packages/*`,
)
