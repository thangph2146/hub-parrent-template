 ...
 *
 * Usage: node script-system/verify/verify-api-profile.cjs hub-parent
 */
...
} = require("../../packages/api-server/deploy/config/render.config.cjs")
...
if (!PRODUCT_LINE_PROFILES[arg]) {
  console.error(
    `Usage: node script-system/verify/verify-api-profile.cjs <${Object.keys(PRODUCT_LINE_PROFILES).join("|")}>`,
  )
  process.exit(1)
}
verify(arg)
