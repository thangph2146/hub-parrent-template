...
function writeGraphStatsMd(
  ...
  lines.push("")
  lines.push(
    `Chạy \`node script-system/graphify-update.cjs apps/${appDir}\` rồi \`pnpm graphify:ai-summary\` (hoặc \`pnpm graphify:refresh\`).`
  )
  lines.push("")
...
function summarizeApp(appDir, label) {
  ...
  lines.push("")
  lines.push(
    `- Cập nhật \`snapshot/context.json\` **và** \`snapshot/graph.json\`: \`node script-system/graphify-update.cjs apps/${appDir}\`.`
  )
  const graphExtras =
    appDir === "api"
...
function writeMonorepoRootSummary() {
  ...
  lines.push("node script-system/graphify-update.cjs apps/frontend")
  lines.push("node script-system/graphify-update.cjs apps/backend")
  lines.push("node script-system/graphify-update.cjs apps/store-sync-frontend")
  lines.push("node script-system/graphify-update.cjs apps/api")
  lines.push(
    "# (Tùy) snapshot graph cấp monorepo — ít node nếu không scan deep"
  )
  lines.push("# node script-system/graphify-update.cjs .")
