 ...
 *
 * Usage:
 *   node script-system/admin/consolidate-admin-loading.cjs
 *   require("./consolidate-admin-loading.cjs").consolidateAdminModuleLoading()
 */
const fs = require("fs")
...
const ROUTING_RE =
  /@workspace\/admin-app\/routing\/admin-(form|detail)-route-loading/

function consolidateAdminModuleLoading({ silent = false } = {}) {
  function walk(dir, rel = "") {
    ...
    for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (ROUTING_RE.test(content)) continue
      let target
      if (content.includes('variant="form"')) target = "admin-form-route-loading"
        ...
        if (!silent) {
          console.warn(
            `[consolidate-admin-loading] bỏ qua (không phải form/detail): ${r}`,
          )
        }
        continue
      }
...
module.exports = { consolidateAdminModuleLoading }
