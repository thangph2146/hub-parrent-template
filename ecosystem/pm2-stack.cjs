 ...
 *
 * Usage: node ecosystem/pm2-stack.cjs <start|restart|delete|reload|stop> <parent|checkin|store>
 */
...
const { ROOT } = require("../script-system/lib/monorepo-root.cjs");
...
/** Tên process cũ — chỉ dùng khi delete để dọn sạch server. */
...
PM2 stack — start | restart | delete (và reload, stop)
  ...
  node ecosystem/pm2-stack.cjs <action> <stack>
...
const jsonPath = path.join(ECOSYSTEM_DIR, `.pm2-ecosystem-${stack}.json`);
