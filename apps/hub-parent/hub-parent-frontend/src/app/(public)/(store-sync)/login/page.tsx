...
function toStoreSession(user: Awaited<ReturnType<typeof api.public.loginWithEmail>>) {
  ...
  return {
    id: String(user.id),
    username: user.email,
    role: isAdmin ? ("admin" as const) : ("store" as const),
...
function LoginFormInner() {
  ...
  const completeLogin = (session: ReturnType<typeof toStoreSession>) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      if (IS_DEV && selectedDevUser) {
        const user = await api.public.loginStoreWithDevelopmentUser({
          userId: String(selectedDevUser.id),
        });
        completeLogin(toStoreSession(user));
        return;
      }
      ...
      const user = await api.public.loginWithEmail({
        email: email.trim(),
        password,
      });
      completeLogin(toStoreSession(user));
