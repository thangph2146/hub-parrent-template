...
> **Sinh tự động:** `2026-06-19T01:42:38.642Z` từ `snapshot/graph.json` (cạnh `relation: "imports"`).
...
| `auth` | `common` | 4 | auth-admin.controller.ts → api-response.ts; auth-admin.controller.ts → public.decorator.ts; auth.controller.ts → common; auth.service.ts → auth.service.ts |
| `auth` | `config` | 2 | auth-admin.controller.ts → constants.ts; auth.controller.ts → constants.ts |
...
- **`common`**: **179** cạnh từ **51** domain — `_root` (8), `orders` (8), `promo-codes` (6), `public` (6), `event-registrations` (5), `academic-years` (4), `auth` (4), `cameras` (4)
- **`config`**: **145** cạnh từ **51** domain — `common` (39), `uploads` (5), `messages` (4), `orders` (3), `products` (3), `promo-codes` (3), `public` (3), `roles` (3)
