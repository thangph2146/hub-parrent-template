...
export function useHandleConfirmAction(del: { mutateAsync: (id: string) => Promise<unknown> }, restore: { mutateAsync: (id: string) => Promise<unknown> }, purge: { mutateAsync: (id: string) => Promise<unknown> }, setConfirmAction: (v: TemplateConfirmAction | null) => void) { return useCallback(async ({ kind, row }: TemplateConfirmAction) => { try { if (kind === "delete") { await del.mutateAsync(row.id); } else if (kind === "restore") { await restore.mutateAsync(row.id); } else { await purge.mutateAsync(row.id); } } catch { /* toast: MutationCache */ } finally { setConfirmAction(null); } }, [del, restore, purge, setConfirmAction]); }
