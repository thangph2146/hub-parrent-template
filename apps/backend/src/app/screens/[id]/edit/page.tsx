...
function EditScreenPageInner() {
  const crudNav = useAdminCrudNavigation("/screens"), params = useParams(), id = params.id as string, qc = useQueryClient(), { form } = useScreenForm();
  ...
  useEffect(() => { if (isError) { toast.error("Không tải được màn hình"); crudNav.list(); } }, [isError, crudNav]);
  ...
  const mut = useMutation({ mutationFn: (i: Record<string, unknown>) => api.screens.update(id, i), onSuccess: async (_d, v) => { await inv(); toast.success(`Đã cập nhật "${v.name}"`); crudNav.view(String(id)); }, onError: (e: Error) => toast.error(e.message || "Lỗi") });
  ...
  if (!e) return null;
  return (<AdminPageSection><ScreenFormShell form={form} onSubmit={h} submitting={mut.isPending} editingId={id} cameraOptions={cameraOptions} templateOptions={templateOptions} onBack={() => crudNav.view(String(id))} onReset={async () => { await refetch(); }} /></AdminPageSection>);
}
