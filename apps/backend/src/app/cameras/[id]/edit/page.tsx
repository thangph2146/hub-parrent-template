...
function EditCameraPageInner() {
  const crudNav = useAdminCrudNavigation("/cameras"), params = useParams(), id = params.id as string, qc = useQueryClient(), { form } = useCameraForm();
  const { data: e, isLoading, isError, refetch } = useCameraDetailQuery(api, id);
  useEffect(() => { if (isError) { toast.error("Không tải được camera"); crudNav.list(); } }, [isError, crudNav]);
  ...
  const mut = useMutation({ mutationFn: (i: Record<string, unknown>) => api.cameras.update(id, i), onSuccess: async (_d, v) => { await inv(); toast.success(`Đã cập nhật "${v.name}"`); crudNav.view(String(id)); }, onError: (e: Error) => toast.error(e.message || "Lỗi") });
  ...
  if (!e) return null;
  return (<AdminPageSection><CameraFormShell form={form} onSubmit={h} submitting={mut.isPending} editingId={id} onBack={() => crudNav.view(String(id))} onReset={async () => { await refetch(); }} /></AdminPageSection>);
}
