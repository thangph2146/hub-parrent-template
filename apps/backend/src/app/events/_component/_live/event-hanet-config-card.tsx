...
export function EventHanetConfigCard({
  ...
  return (
    <Collapsible defaultOpen={false} className="w-full">
      <Card className="border border-dashed border-primary/30 bg-primary/5 shadow-sm justify-start py-0 gap-0">
        <CollapsibleTrigger className="group w-full cursor-pointer text-left transition-colors hover:bg-primary/10 py-2">
          <CardHeader>
            <CardTitle className="flex flex-wrap items-center gap-2 text-base">
              ...
              <span className="w-full text-xs font-normal text-muted-foreground sm:w-auto sm:ml-auto">
                {hasCameras
        ...
        <div className="rounded-md border border-border/70 bg-background px-3 py-2.5 text-xs text-muted-foreground">
          <p className="font-medium text-foreground">Yêu cầu (theo HANET)</p>
          <ul className="mt-2 list-inside list-disc space-y-1">
            ...
            <li>
              API HUB public (HTTPS) — HANET gọi được từ internet; URL lấy từ{" "}
              <code className="text-[10px]">NEXT_PUBLIC_API_URL</code>.
            </li>
            <li>
              Mã thiết bị <code className="text-[10px]">deviceID</code> trên
              payload = <strong>mã camera</strong> trong HUB (nhập tay khi tạo
              camera).
            </li>
            <li>
              Người quét phải đã <strong>đăng ký sự kiện</strong> (email / mã
              người khớp danh sách).
            </li>
          </ul>
          <p className="mt-2 text-[11px]">
            OAuth2 (<code className="text-[10px]">client_id</code> /{" "}
            <code className="text-[10px]">access_token</code>) chỉ cần khi gọi
            HTTP API HANET — webhook realtime không bắt buộc.
          </p>
        </div>
        ...
        <div className="space-y-1">
          ...
          <ol className="list-inside list-decimal space-y-1 text-xs text-muted-foreground">
            <li>
              ...
              — nhập <strong>Mã camera (deviceID HANET)</strong> đúng như trên
              cổng HANET (có thể gắn thêm sự kiện mặc định ở camera).
            </li>
            <li>
              <Link
