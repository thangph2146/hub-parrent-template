...
> **Sinh tự động:** `2026-06-19T01:42:38.386Z` từ `snapshot/graph.json` (cạnh `relation: "imports"`).
...
| `common` | `config` | 36 | base-admin-http.controller.ts → constants.ts; api-access.middleware.ts → constants.ts; logging.interceptor.ts → app.config.ts; logging.interceptor.ts → constants.ts |
| `common` | `entities` | 7 | dev-login-options.ts → role.entity.ts; dev-login-options.ts → user.entity.ts; dev-login-options.ts → user-role.entity.ts; gift-rules.spec.ts → product.entity.ts |
| `common` | `socket` | 3 | broadcast.service.ts → socket.gateway.ts; broadcast.service.ts → socket.types.ts; interceptor.ts → socket.gateway.ts |
...
| `parent-students` | `common` | 4 | parent-students.controller.ts → common; parent-students.service.spec.ts → broadcast.service.ts; parent-students.service.ts → broadcast.service.ts; parent-students.service.ts → parent-student.service.ts |
...
| `public` | `common` | 6 | public-auth.service.ts → common; public-contact-requests.service.ts → broadcast.service.ts; public-event-registration.service.ts → common; public-events.service.ts → common |
...
| `socket` | `common` | 3 | socket.gateway.ts → common; socket.module.ts → interceptor.ts; socket.module.ts → broadcast.service.ts |
...
- **`entities`**: **211** cạnh từ **48** domain — `mikro-orm` (46), `_root` (27), `public` (15), `seeds` (14), `groups` (8), `auth` (7), `common` (7), `messages` (6)
...
- **`config`**: **141** cạnh từ **51** domain — `common` (36), `uploads` (5), `messages` (4), `orders` (3), `products` (3), `promo-codes` (3), `public` (3), `roles` (3)
