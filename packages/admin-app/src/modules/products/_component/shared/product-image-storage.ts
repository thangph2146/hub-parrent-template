import { resolveStorageFolderSlugPath, type StoreSyncSdk } from "@workspace/api-client"
import {
  createStorageFolder,
  fetchStorageFolders,
  type FolderItem,
...
export async function ensureProductImageFolder(
  ...
  let folders = await fetchStorageFolders(api)
  const existing = findProductImageFolder(folders, input)
  if (existing) {
    return { ...existing, created: false }
  }

  await ensureProductImageParentFolder(api, folders)
  folders = await fetchStorageFolders(api)
  ...
  const created = await createStorageFolder(api, {
