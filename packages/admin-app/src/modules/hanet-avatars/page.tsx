...
import { HanetAvatarCard, useHanetAvatarsQuery } from "./_component"
import { HanetPlaceSelect } from "./_component/hanet-place-select"
import { readHanetAdminPlaceId } from "@workspace/admin-app/lib/hanet-place-storage"

const PAGE_SIZE = 50

function HanetAvatarsPageInner() {
  ...
  return (
    <AdminPageSection>
      <AdminListPageHeader
        icon={ScanFace}
        title="Avatar HANET"
        subtitle="Danh sách khuôn mặt đã đăng ký trên HANET — lưu local trong face_data."
        actions={
          <AdminPageHeaderOutlineButton
            disabled={
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border/70 bg-muted/20 px-4 py-3 text-sm">
          <p className="text-muted-foreground">
            <span className="font-medium text-foreground">{total}</span> avatar
            đã lưu
            {items.length < total ? (
              <>
                {" "}
            ...
            {hanetStatus?.defaultPlaceId ? (
              <>
                {" "}
                · place{" "}
                <code className="text-xs">{hanetStatus.defaultPlaceId}</code>
              </>
            ) : null}
          </p>
          {!hanetStatus?.configured ? (
            <p className="text-xs text-amber-700 dark:text-amber-400">
              Chưa cấu hình OAuth HANET trong .env API — chỉ xem bản ghi webhook /
              đăng ký SV.
            </p>
          ) : null}
          <Link
        ...
        {hanetStatus?.configured && !effectivePlaceId ? (
          <p className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
            Chọn <strong>địa điểm HANET</strong> bên dưới rồi bấm &quot;Đồng bộ từ
            HANET&quot;. Mỗi place có danh sách người riêng — ví dụ cơ sở Thủ Đức
            thường có nhiều avatar hơn các cơ sở mới.
          </p>
        ) : null}
        ...
        {listQuery.isLoading ? (
          ...
          <div className="rounded-lg border border-dashed border-border/80 py-16 text-center text-sm text-muted-foreground">
            {effectivePlaceId ? (
              <>
                Chưa có avatar cho place{" "}
                <code className="text-xs">{effectivePlaceId}</code> — thử place
                khác hoặc bấm &quot;Đồng bộ từ HANET&quot;.
              </>
            ) : (
              <>
                Chưa có avatar — chọn địa điểm, bấm &quot;Đồng bộ từ HANET&quot;,
                đăng ký SV có ảnh, hoặc bật webhook person trên cổng HANET.
              </>
            )}
          </div>
        )}
...
export default function HanetAvatarsPage() {
  return (
    <AdminPageGuard
      permissions={[
