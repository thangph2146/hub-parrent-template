...
import { TrainingLevelsTable, TrainingLevelsTrashTable } from "../_table"
import { getTrainingLevelColumns } from "../_table/columns"
import { useColumnFiltersChange, useClearListFilters, useClearTrashFilters } from "../_hooks"
import { useTrainingLevelsListQuery, useTrainingLevelsTrashQuery, prefetchTrainingLevelDetail } from "../_query"
...
export function TrainingLevelsPageInner() {
  const api = useAdminApi()
  const queryClient = useQueryClient()
  const crudNav = useAdminModuleNavigation("training-levels", {
    prefetchDetail: (id) => prefetchTrainingLevelDetail(queryClient, api, id),
  })
  const { user } = useAuth()
  const canWrite = user
  ...
  const listFilterParams = useMemo(
    () =>
      buildAdminFilterQuery(
        columnFilters,
        COMMON_FILTER_MAPPINGS.trainingLevels
      ),
    [columnFilters]
  )
  ...
  const listQuery = useTrainingLevelsListQuery(
    api,
    canWrite || true,
    listFilterParams
  )
  ...
  const deleteMutation = useAdminMutation({
    mutationKey: ["training-levels", "delete"],
  ...
  const restoreMutation = useAdminMutation({
    mutationKey: ["training-levels", "restore"],
  ...
  const purgeMutation = useAdminMutation({
    mutationKey: ["training-levels", "purge"],
  ...
  const rowActions = useAdminCrudRowHandlers<TrainingLevelRow>({
      ...
      <AdminListPageHeader
        ...
        readOnlyHint={
          user && !canWrite ? (
            <AdminReadOnlyHint>
              Chỉ xem: cần quyền{" "}
              <span className="font-mono">training_levels:manage</span> để
              thêm/sửa/xoá.
            </AdminReadOnlyHint>
          ) : undefined
        }
