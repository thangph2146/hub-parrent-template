...
import type { YearAverage } from "@workspace/admin-app/types/student-scores"
import { formatScore } from "../shared/score-utils"
...
export const YearAveragesList = ({ averages, isLoading }: Props) => {
  const sorted = useMemo(
    () =>
      averages
        ? [...averages].sort((a, b) =>
            (b.yearStudy || "").localeCompare(a.yearStudy || "")
          )
        : [],
    [averages]
  )
    ...
    () => [
      {
        ...
        header: "Năm học",
        cell: ({ row }) => (
          <span className="font-medium">{row.original.yearStudy}</span>
        ),
      },
      {
  ...
  if (!sorted.length) {
    return (
      <div className="py-8 text-center text-sm text-muted-foreground">
        Chưa có dữ liệu điểm trung bình theo năm
      </div>
    )
  }
