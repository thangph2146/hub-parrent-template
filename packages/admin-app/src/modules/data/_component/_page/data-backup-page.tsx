...
import { getApiBaseUrl } from "@workspace/admin-app/lib/api-base-url"
...
import { EntitySchemaPanel } from "../system/entity-schema-panel"
import {
  pickExportDirectory,
  saveExportBlob,
...
export function DataBackupPageInner() {
  const { user } = useAuth()
  ...
  const exportJson = async (): Promise<void> => {
    ...
    setExporting("json")
    try {
      const res = await fetch(`${getApiBaseUrl()}/admin/system/export`, {
        headers: authHeaders(),
      })
      if (!res.ok) {
  ...
  const exportExcel = async (): Promise<void> => {
    ...
    setExporting("excel")
    try {
      const res = await fetch(`${getApiBaseUrl()}/admin/system/export/excel`, {
        headers: authHeaders(),
      })
      if (!res.ok) {
        await toastFetchError(res)
