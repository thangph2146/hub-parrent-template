...
import {
  ...
  STORAGE_ROOT_IMAGES,
  STORAGE_ROOT_VIDEOS,
} from "../browse/file-storage-folder-tree"
import {
...
export function FileStorageUploadDialog({
  ...
  const loadFolders = useCallback(async () => {
    ...
    try {
      const list = await fetchStorageFolders(api)
      setFolders(list)
  ...
  }, [api, activeFolderPath, activeRealm])
  ...
  const runUpload = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        for (const file of files) {
          try {
            await uploadAdminImage(api, file, { folderPath, isExistingFolder })
            success++
          } catch {
            fail++
          }
    ...
    [api, onOpenChange, onUploaded, onUploadingChange, resolveUploadFolder]
  )
