/**
 * Controller spec — BaseEventSpeakersController (unified admin HTTP).
...
import type { IEventSpeakersControllerService } from './event-speakers.controller';
...
function createServiceMock(): jest.Mocked<IEventSpeakersControllerService> {
  return {
    list: jest.fn(),
    getById: jest.fn(),
    create: jest.fn(),
    ...
    delete: jest.fn(),
    bulk: jest.fn(),
  } as jest.Mocked<IEventSpeakersControllerService>;
}

describe('BaseEventSpeakersController — unified admin contract', () => {
  let controller: BaseEventSpeakersController;
  let service: jest.Mocked<IEventSpeakersControllerService>;
  const headers = { 'x-user-id': '7' };
  const handlers = ['list', 'getById', 'create', 'update', 'delete', 'bulk'] as const;

  beforeEach(() => {
    service = createServiceMock();
    controller = new BaseEventSpeakersController(service);
  });
