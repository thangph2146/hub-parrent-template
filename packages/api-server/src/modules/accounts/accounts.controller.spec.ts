/**
 * Controller spec — BaseAccountsController (unified admin HTTP).
...
import type { IAccountsControllerService } from './accounts.controller';
...
function createServiceMock(): jest.Mocked<IAccountsControllerService> {
  return {
    getProfile: jest.fn(),
    updateProfile: jest.fn(),
  } as jest.Mocked<IAccountsControllerService>;
}

describe('BaseAccountsController — unified admin contract', () => {
  let controller: BaseAccountsController;
  let service: jest.Mocked<IAccountsControllerService>;
  const headers = { 'x-user-id': '7' };
  const handlers = ['getProfile', 'updateProfile', 'uploadAvatar'] as const;

  beforeEach(() => {
    service = createServiceMock();
    controller = new BaseAccountsController(service, {} as never);
  });
