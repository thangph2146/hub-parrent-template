...
export type { CreateStorageFolderResult, UploadFileInput } from './uploads-disk.service';
...
@Injectable()
export abstract class BaseUploadsService {
  ...
  private async ensureStorageFileUploader(
    ...
    const existing = await this.getEm().findOne(this.getStorageFileEntity(), {
      relativePath: normalized,
    });
    if (existing) return;

    const now = new Date();
    const record = this.getEm().create(this.getStorageFileEntity(), {
      relativePath: normalized,
      uploadedBy: this.getEm().getReference(this.getUserEntity(), toEntityId(uploader)),
      createdAt: now,
      updatedAt: now,
    });
    await this.getEm().persistAndFlush(record);
  }

  private async deleteStorageFileMeta(relativePath: string): Promise<void> {
    const normalized = this.normalizeStorageRelativePath(relativePath);
    await this.getEm().nativeDelete(this.getStorageFileEntity(), { relativePath: normalized });
  }
  ...
  private async moveStorageFileMeta(from: string, to: string): Promise<void> {
    ...
    const record = await this.getEm().findOne(this.getStorageFileEntity(), {
  ...
  private async resolveUploadOwnerLabels(
    ...
    const users = await this.getEm().find(
      this.getUserEntity(),
  ...
  private async scanLegacyStorageDirs(
    ...
    try {
      const top = await readdir(this.getStorageDir(), { withFileTypes: true });
      for (const entry of top) {
        if (!entry.isDirectory() || STORAGE_LEGACY_SKIP_DIRS.has(entry.name)) {
          continue;
        }
        const full = path.join(this.getStorageDir(), entry.name);
  ...
  async collectAllStorageItems(serveBaseUrl = ''): Promise<ImageItemDto[]> {
    ...
    allImages.push(
      ...(await this.scanImagesInDir(this.getImagesDir(), 'images', baseUrl)),
    );
    allImages.push(
      ...(await this.scanImagesInDir(this.getFilesDir(), 'files', baseUrl)),
    );
    allImages.push(
      ...(await this.scanImagesInDir(this.getVideosDir(), 'videos', baseUrl)),
      ...(await this.scanImagesInDir(this.getAudioDir(), 'audio', baseUrl)),
    );
  ...
  async listFolders(): Promise<ListFoldersResult> {
    ...
    await addFromImages(this.getImagesDir(), 'images');
    await addFromImages(this.getFilesDir(), 'files');
    await addFromImages(this.getVideosDir(), 'videos');
    await addFromImages(this.getAudioDir(), 'audio');
    const realmRoots: Array<[string, string]> = [
      [this.getImagesDir(), 'images'],
      [this.getFilesDir(), 'files'],
      [this.getVideosDir(), 'videos'],
      [this.getAudioDir(), 'audio'],
    ];
    ...
    try {
      ...
      for (const entry of top) {
        ...
        await addFromImages(path.join(this.getStorageDir(), entry.name), entry.name);
      }
    } catch {
      // ignore
    }
  ...
  private async readPolicyWalkingUp(
    ...
    const uploadsRoot = path.resolve(this.getUploadsDir());
    ...
    if (parentPath) {
      ...
      if (realmTarget) {
        ...
        if (clean.startsWith('images') || this.hasDateStructure(clean)) {
          ...
          targetDir = path.join(this.getStorageDir(), clean, safeName);
      ...
      if (resourceType === 'files') {
        targetDir = path.join(this.getFilesDir(), safeName);
        folderPath = path.join('files', safeName).replace(/\\/g, '/');
      } else if (resourceType === 'videos') {
        targetDir = path.join(this.getVideosDir(), safeName);
        folderPath = path.join('videos', safeName).replace(/\\/g, '/');
      } else if (resourceType === 'audio') {
        targetDir = path.join(this.getAudioDir(), safeName);
        folderPath = path.join('audio', safeName).replace(/\\/g, '/');
      } else {
        targetDir = path.join(this.getStorageDir(), safeName);
        folderPath = safeName;
      }
    }
  ...
  async deleteFolder(relativePath: string): Promise<void> {
    ...
    if (clean.startsWith('images/')) {
      fullPath = path.resolve(this.getImagesDir(), clean.slice(7));
      baseDir = path.resolve(this.getImagesDir());
    } else if (clean.startsWith('files/')) {
      fullPath = path.resolve(this.getFilesDir(), clean.slice(6));
      baseDir = path.resolve(this.getFilesDir());
    } else if (clean.startsWith('videos/')) {
      fullPath = path.resolve(this.getVideosDir(), clean.slice(7));
      baseDir = path.resolve(this.getVideosDir());
    } else if (clean.startsWith('audio/')) {
      fullPath = path.resolve(this.getAudioDir(), clean.slice(6));
      baseDir = path.resolve(this.getAudioDir());
    } else {
      fullPath = path.resolve(this.getStorageDir(), clean);
      baseDir = path.resolve(this.getStorageDir());
    }
  ...
  async serveResized(
    ...
    const cacheDir = path.join(this.getStorageDir(), IMAGE_RESIZE_CACHE_DIR);
...
/** @deprecated Dùng `BaseUploadsService`. */
export abstract class BaseUploadsAdminService extends BaseUploadsService {}
