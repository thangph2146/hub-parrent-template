...
function classifyStorageMedia(ext: string, mimeType: string): StorageMediaKind {
  const normalizedExt = ext.toLowerCase();
  const mime = mimeType.toLowerCase();
  if (
    ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.bmp', '.tif', '.tiff', '.heic', '.heif'].includes(
      normalizedExt,
    ) ||
  ...
  if (['.mp4', '.mov', '.avi', '.m4v', '.webm'].includes(normalizedExt) || mime.startsWith('video/')) {
    return 'video';
  }
  if (['.mp3', '.wav'].includes(normalizedExt) || mime.startsWith('audio/')) {
    return 'audio';
  }
  if (['.zip', '.rar', '.7z'].includes(normalizedExt)) {
    return 'archive';
  }
  if (['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.csv', '.rtf', '.txt', '.ppt', '.pptx'].includes(normalizedExt)) {
    return 'document';
  }
  return 'other';
}

function storageRealmFromPath(relativePath: string, mediaKind: StorageMediaKind): StorageRealm {
  const normalized = normalizeSlashes(relativePath);
  if (normalized.startsWith('audio/')) return 'audio';
  if (normalized.startsWith('videos/')) return 'videos';
  if (normalized.startsWith('files/')) return 'files';
  if (normalized.startsWith('images/')) return 'images';
...
function storageTabFromPath(relativePath: string): string {
  const parts = normalizeSlashes(relativePath).split('/').filter(Boolean);
  if (!parts.length) return 'other';
  if (STORAGE_REALMS.includes(parts[0] as StorageRealm)) {
...
@Injectable()
export class BaseUploadsService {
  ...
  protected async scanDir(
    ...
    for (const entry of entries) {
      ...
      out.push({
        ...
        mediaKind,
        storageTab: storageTabFromPath(nextRelative),
        storageRealm: storageRealmFromPath(nextRelative, mediaKind),
        uploadOwnerId,
  ...
  async listImages(params: {
    ...
    const tabs = params.realm
      ? [...new Set(realmItems.map((item) => storageTabFromPath(item.relativePath)))]
          .sort((left, right) => left.localeCompare(right))
          .map((id) => ({
            id,
            label: titleize(id),
            count: realmItems.filter((item) => storageTabFromPath(item.relativePath) === id).length,
          }))
      : [];
  ...
  async saveFile(
    ...
    const realm = storageRealmFromPath(originalName, mediaKind);
