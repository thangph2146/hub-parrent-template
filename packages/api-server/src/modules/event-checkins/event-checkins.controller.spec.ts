/**
 * Controller spec — BaseEventCheckinsController (unified admin HTTP).
...
import type { IEventCheckinsControllerService } from './event-checkins.controller';
...
function createServiceMock(): jest.Mocked<IEventCheckinsControllerService> {
  return {
    list: jest.fn(),
    getById: jest.fn(),
    create: jest.fn(),
    ...
    restore: jest.fn(),
    bulk: jest.fn(),
  } as jest.Mocked<IEventCheckinsControllerService>;
}

describe('BaseEventCheckinsController — unified admin contract', () => {
  let controller: BaseEventCheckinsController;
  let service: jest.Mocked<IEventCheckinsControllerService>;
  const headers = { 'x-user-id': '7' };
  const handlers = ['list', 'getById', 'create', 'update', 'hardDelete', 'softDelete', 'restore', 'bulk'] as const;

  beforeEach(() => {
    service = createServiceMock();
    controller = new BaseEventCheckinsController(service);
  });
