/**
 * Controller spec — BaseEventRegistrationsController (unified admin HTTP).
...
import type { IEventRegistrationsControllerService } from './event-registrations.controller';
...
function createServiceMock(): jest.Mocked<IEventRegistrationsControllerService> {
  return {
    syncEventRegistrationCount: jest.fn(),
    list: jest.fn(),
    getById: jest.fn(),
    create: jest.fn(),
    update: jest.fn(),
    ...
    restore: jest.fn(),
    bulk: jest.fn(),
  } as jest.Mocked<IEventRegistrationsControllerService>;
}

describe('BaseEventRegistrationsController — unified admin contract', () => {
  let controller: BaseEventRegistrationsController;
  let service: jest.Mocked<IEventRegistrationsControllerService>;
  const headers = { 'x-user-id': '7' };
  const handlers = ['list', 'getById', 'create', 'setAttendance', 'update', 'hardDelete', 'softDelete', 'restore', 'bulk'] as const;

  beforeEach(() => {
    service = createServiceMock();
    controller = new BaseEventRegistrationsController(service, {} as never);
  });
