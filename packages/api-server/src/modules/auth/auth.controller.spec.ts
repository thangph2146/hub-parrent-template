...
describe('BaseAuthController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new BaseAuthController(service as never);
  });
  ...
  it('logout tra success envelope', async () => {
    service.logout.mockResolvedValue({ ok: true });
    const res = createResponseMock();
    await controller.logout({ userId: '7' }, res as never);
    expect(service.logout).toHaveBeenCalledWith('7');
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.payload).toEqual(
      expect.objectContaining({
        success: true,
