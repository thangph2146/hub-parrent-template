...
export function reportImportRowError(
...
export type ExportPivotSanitizeCounts = ReturnType<
  typeof sanitizePivotRowsInExportJson
>;

/** Ghi log các pivot bị loại khi export JSON tự nhất quán. */
export function logExportPivotSanitizeWarnings(
  dropped: ExportPivotSanitizeCounts,
  warn: (message: string) => void,
): void {
  if (dropped.droppedPostCategory > 0) {
    warn(
      `Export: loại ${dropped.droppedPostCategory} postCategory trỏ tới post/category không có trong cùng file export.`,
    );
  }
  if (dropped.droppedPostTag > 0) {
    warn(
      `Export: loại ${dropped.droppedPostTag} postTag trỏ tới post/tag không có trong cùng file export.`,
    );
  }
  if (dropped.droppedEventSpeaker > 0) {
    warn(
      `Export: loại ${dropped.droppedEventSpeaker} eventSpeaker trỏ tới event/speaker không có trong cùng file export.`,
    );
  }
  if (dropped.droppedEventRegistration > 0) {
    warn(
      `Export: loại ${dropped.droppedEventRegistration} eventRegistration trỏ tới event không có trong cùng file export.`,
    );
  }
  if (dropped.droppedEventCheckin > 0) {
    warn(
      `Export: loại ${dropped.droppedEventCheckin} eventCheckin trỏ tới event không có trong cùng file export.`,
    );
  }
  if (dropped.droppedGroupMember > 0) {
    warn(
      `Export: loại ${dropped.droppedGroupMember} groupMember trỏ tới group/user không có trong cùng file export.`,
    );
  }
  if (dropped.droppedMessageRead > 0) {
    warn(
      `Export: loại ${dropped.droppedMessageRead} messageRead trỏ tới message/user không có trong cùng file export.`,
    );
  }
  if (dropped.droppedUserRole > 0) {
    warn(
      `Export: loại ${dropped.droppedUserRole} userRole trỏ tới user/role không có trong cùng file export.`,
    );
  }
}
