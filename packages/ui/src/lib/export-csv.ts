...
export type CsvExportOptions = {
  ...
  /** Các dòng mô tả đặt trước header, ví dụ tiêu đề báo cáo/ngày xuất/chủ đề. */
  preambleRows?: string[][]
}
...
export function rowsToCsvContent(
  ...
  const lines = [
    ...preambleRows.map((r) => r.map(esc).join(delimiter)),
    ...(preambleRows.length ? [""] : []),
...
export function csvToBlobParts(
  ...
  const body = rowsToCsvContent(headers, rows, delimiter, options?.preambleRows)
